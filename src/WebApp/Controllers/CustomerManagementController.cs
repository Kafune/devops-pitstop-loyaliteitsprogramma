...
public class CustomerManagementController : Controller
{
    ...
    public CustomerManagementController(ICustomerManagementAPI customerManagementAPI, ILoyaltySystemAPI loyaltySystemAPI, ILogger<CustomerManagementController> logger)
    {
    ...
    public async Task<IActionResult> Register([FromForm] CustomerManagementNewViewModel inputModel)
    {
        if (ModelState.IsValid)
        {
            return await _resiliencyHelper.ExecuteResilient(async () =>
            {
                ...
                RegisterLoyaltyCustomer loyaltycmd = new RegisterLoyaltyCustomer(Guid.NewGuid(), cmd.CustomerId);
                await _loyaltySystemAPI.AddCustomer(loyaltycmd);
